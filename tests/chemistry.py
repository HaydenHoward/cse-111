def main():
    ...
    compound = get_formula_name(formula, known_molecules_dict)
...
def get_formula_name(formula, known_molecules_dict):
    ...
    if formula in known_molecules_dict:
        element = known_molecules_dict[formula]
        return element
    else:
        unkown = "unkown compound"
        return unkown
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for i in symbol_quantity_list:
        ...
        if i[0] in periodic_table_dict:
            values = periodic_table_dict[i[0]]
            atomic_mass = values[1]
            quantity = i[1]
            total_mass += atomic_mass * quantity

# dont use
    # for element, quantity in symbol_quantity_list:
    #     if element in periodic_table_dict.items():
    #         atomic_mass = periodic_table_dict[ATOMIC_MASS_INDEX]
    #         total_mass += atomic_mass * quantity

    # for periodic_key, periodic_list in periodic_table_dict.items():
    #     if symbol_quantity_list[SYMBOL_INDEX] in periodic_key:
    #         quantity = symbol_quantity_list[QUANTITY_INDEX]
    #         atomic_mass = periodic_list[ATOMIC_MASS_INDEX]
    #         total_mass += atomic_mass * quantity
